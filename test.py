...
def preprocess_frames(frames):
    ...
    log_mels = np.log2(mels, dtype=np.float32) # Multiply by 8 (shift by 3) to get a reasonably sized output  approx (0-160)
...
res = preprocess((sine_wave + 1) * 2047 * 0.8)
with np.printoptions(linewidth=104):
    print(res.transpose())
